import type { InventoryItems } from '@tips/bungieTypes/manifest.js'
...
export const exoticWeaponLinking = (data: PerkDataList, invItems: InventoryItems) => {
  const linkedByDescription = Object.values(data).reduce(
    (acc, perk) => {
...
export const weaponFrameLinking = (data: PerkDataList) => {
...
export const originTraitLinking = (data: PerkDataList) => {
  const linkedByName: { [key: string]: number[] } = {}
...
export const armorModLinking = (data: PerkDataList) => {
  const linkedByName: { [key: string]: number[] } = {}
